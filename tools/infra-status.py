...
def check_open_ports() -> str:
    ...
    external, local_only_count = classify_external_ports(output)
    if not external:
        return f'No externally exposed listening ports (local-only listeners: {local_only_count})'
    ...
    dedup = sorted(external)
    preview = ', '.join(dedup[:8])
    if len(dedup) > 8:
        preview += ', ...'
    ...
    return f'Externally exposed listeners match allowlist ({len(dedup)}): {preview}'
...
def count_failed_auth_attempts(log_text: str) -> int:
    pattern = re.compile(r'(failed|invalid user|authentication failure|error: pam)', re.IGNORECASE)
    return sum(1 for line in log_text.splitlines() if pattern.search(line))
