...
def has_open_files(path: Path) -> str | None:
    if shutil.which('lsof') is None:
        return 'open-file check unavailable (lsof missing)'
    proc = subprocess.run(['lsof', str(path)], capture_output=True, text=True, timeout=45, check=False)
    if (proc.stdout or '').strip():
        return 'open files detected'
    return None
...
def is_workspace_log_path(path: Path) -> bool:
    try:
        relative = normalize_path(path).relative_to(LOG_ROOT)
    except ValueError:
        return False
    return len(relative.parts) == 1 and relative.name.startswith('night-') and relative.suffix == '.log'


def iter_allowlisted_paths() -> list[Path]:
    if not LOG_ROOT.exists():
        return []
    return sorted(path for path in LOG_ROOT.glob(ALLOWLIST_GLOB) if path.is_file())
...
def validate_candidate(path: Path, *, min_bytes: int) -> tuple[CleanupCandidate | None, str | None]:
    ...
    if not is_workspace_log_path(normalized):
        return None, 'not an allowlisted workspace log path'
    ...
    open_file_reason = has_open_files(normalized)
    if open_file_reason is not None:
        return None, open_file_reason
...
def list_mode(min_bytes: int, limit: int) -> int:
    ...
    if not candidates:
        print('WORKSPACE_LOG_CLEANUP_NO_ELIGIBLE_CANDIDATES')
        return 0

    print('WORKSPACE_LOG_CLEANUP_REVIEW')
    ...
    if blocked:
        ...
        print('Run cleanup from a shell with write access to these workspace log paths before using --apply')
    if applyable:
        print(f"Suggested apply: python3 tools/infra_workspace_log_cleanup.py --apply {suggested_apply_args(applyable)}")
    else:
...
def apply_mode(paths: list[str], min_bytes: int) -> int:
    if not paths:
        print('WORKSPACE_LOG_CLEANUP_APPLY_REQUIRES_PATHS', file=sys.stderr)
        return 2
    ...
    print('WORKSPACE_LOG_CLEANUP_RESULT')
...
def review_paths(paths: list[str], min_bytes: int) -> int:
    ...
    if suggested:
        print(f"Suggested apply: python3 tools/infra_workspace_log_cleanup.py --apply {suggested}")
    else:
        print('Suggested apply unavailable from current session')
    return 0
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Safe workspace log cleanup helper')
    parser.add_argument('--apply', action='store_true', help='remove the provided paths after validation')
    parser.add_argument('--path', action='append', default=[], help='allowlisted workspace log path to review/remove')
