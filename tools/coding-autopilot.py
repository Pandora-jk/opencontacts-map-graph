...
def build_status(state: dict, card: dict[str, str] | None, artifact: Path | None, total_ready: int) -> tuple[str, str]:
...
def main() -> int:
    ...
    if ready:
        ...
        if card is None:
            ...
            status_text, telegram_text = build_status(state, None, None, len(ready))
    ...
    status_text, telegram_text = build_status(state, card, artifact, len(ready))
    STATUS_FILE.write_text(status_text, encoding='utf-8')
    save_state(state)
