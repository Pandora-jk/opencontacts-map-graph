...
- [ ] Fix the coding-day-loop blocker caused by GitHub secret scanning.
  ...
  Additional progress applied on 2026-03-07 14:21 UTC:
  - added [coding-clean-branch.sh](/home/ubuntu/.openclaw/workspace/tools/coding-clean-branch.sh) to build a clean orphan snapshot branch from the current sanitized tree in a separate worktree while omitting generated runtime files; the helper now defaults to a pushable `feature/clean-snapshot-*` branch name to match the repo’s branch policy
  - created local clean snapshot worktree at `/tmp/openclaw-workspace-clean-20260307-1420` with root commit `fcc9d75`
  - pushed the verified clean snapshot to `origin/feature/workspace-clean-snapshot-20260307-1420`
  Verification:
  ...
  - `python3 tools/git-secret-scan.py --repo /tmp/openclaw-workspace-clean-20260307-1420` returns `SECRET_SCAN_OK`
  - `python3 tools/git-secret-scan.py --repo /tmp/openclaw-workspace-clean-20260307-1420 --history-ref HEAD --max-commits 20` also returns `SECRET_SCAN_OK`
  - `git ls-remote --heads origin feature/workspace-clean-snapshot-20260307-1420` resolves to `fcc9d75169e6f051df25a5d700d4c4f256e69ab7`
  - the current feature branch also has no merge base with `origin/main`, so a normal rebase cleanup path is not available
  Remaining fix: switch the active automation path onto the clean feature branch (or refresh the main workspace from it) so future autonomous pushes stop originating from the poisoned local branch.
...
- [x] Make `Night Coding Delegation` degrade gracefully when Codex transport is unavailable.
  Applied: updated [night-department-runner.sh](/home/ubuntu/.openclaw/workspace/tools/night-department-runner.sh) to fall back to [coding-autopilot.py](/home/ubuntu/.openclaw/workspace/tools/coding-autopilot.py) when no dev CLI is available or Codex transport fails with websocket/HTTPS connection errors.
  Verification: `python3 tools/department-commands.py run coding` during Sydney night now returns `NIGHT_CODING_OK cli=deterministic fallback=llm_transport_unavailable`; `PATH=/usr/bin:/bin tools/night-department-runner.sh coding` also returns `NIGHT_CODING_OK cli=deterministic fallback=no_dev_cli`; and [night-coding.log](/home/ubuntu/.openclaw/workspace/logs/night-coding.log) records the deterministic coding update immediately after the fallback trigger.
