# Finance Department - Team Directory

**Department:** Finance
**Head:** Finance Head
**Mission:** Sell one real service, keep one side-cash lane active, and avoid parallel idea drift.

## Team Roster

| Role | Status | Domain | Current Focus |
|------|--------|--------|---------------|
| Finance Head | ACTIVE | Revenue orchestration | Keep the department focused on one core service and one side lane |
| Legal Expert | ACTIVE | Compliance | Review outreach, suppression, consent, and complaint handling |
| Revenue Service Owner | ACTIVE | B2B pipeline setup service | ICP, list quality, outreach quality, demos, closes |
| GitHub Bounties Owner | ACTIVE | Side cash | Screen and attempt narrowly matched bounties |
| Website Upsell Lane | INCUBATING | Conversion support | Only use when it helps close or retain a pipeline client |
| Crypto | PAUSED | Speculative capital deployment | No execution |
| Data Brokerage | PAUSED | Bulk data product ideas | No execution |
| Skills and templates | PAUSED | Productization | Resume only after repeated service demand |

## Reporting Structure

```
Jim Knopf
|
`-- Finance Head
    |-- Revenue Service Owner
    |-- GitHub Bounties Owner
    |-- Legal Expert
    `-- Website Upsell Lane (incubating only)
```

## Decision Rules

- The core service has priority over every other lane.
- Bounties can run only if they do not slow the core service.
- Upsells must support the core service sale or delivery.
- Paused lanes do not consume autonomous finance cycles.

## Review Cadence

- Daily: core service KPI check
- Weekly: lane review and pause-or-continue decision
- Monthly: only consider new products if service traction is proven

**Last Updated:** 2026-03-08T07:00 UTC
