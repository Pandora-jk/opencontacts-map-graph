...
- [x] **Write unit tests for `pdf_to_csv.py` core conversion path**
  ...
  - test-results: `python3 -m unittest tests/test_pdf_to_csv.py -v` => 8 passed; `python3 -m coverage report -m pdf_to_csv.py` => 73% coverage (missing: import error handling, exception handling)
  - notes: Coverage achieved (73%) - core conversion path well tested; remaining 27% is error handling paths (import failure, exception handling)
