...
- UTC: 2026-03-10T03:16:36.819721Z

## Disk Usage
Root usage: /: 100% used (19G/19G, avail 136M)
...
- Need about 1.7G reclaimed to reach <=90% on /
  All writable stale /tmp paths total 87M across 141 path(s); short by 1.6G
  Review top stale /tmp paths: python3 tools/infra_tmp_cleanup.py --limit 5
  Current scan found 141 writable stale /tmp path(s); rerun the helper with a higher --limit or targeted --path values before cleanup
  Host-level reclaim is still required after current-session cleanup
- Need about 3.5G reclaimed to reach <=80% on /
  All writable stale /tmp paths total 87M across 141 path(s); short by 3.4G
  Review top stale /tmp paths: python3 tools/infra_tmp_cleanup.py --limit 5
  Current scan found 141 writable stale /tmp path(s); rerun the helper with a higher --limit or targeted --path values before cleanup
  Host-level reclaim is still required after current-session cleanup
...
- Need about 3.5G reclaimed to reach <=80% on /
  All allowlisted home caches total 2.5G across 4 path(s); short by 1006M
