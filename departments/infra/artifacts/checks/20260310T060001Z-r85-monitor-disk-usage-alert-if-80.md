...
- UTC: 2026-03-10T06:00:01.407978Z

## Disk Usage
Root usage: /: 99% used (19G/19G, avail 192M)
...
- 642M /var/cache/apt (APT package cache)
...
- Need about 1.6G reclaimed to reach <=90% on /
  All writable stale /tmp paths total 15M across 136 path(s); short by 1.6G
  Review top stale /tmp paths: python3 tools/infra_tmp_cleanup.py --limit 5
  Current scan found 136 writable stale /tmp path(s); rerun the helper with a higher --limit or targeted --path values before cleanup
  Host-level reclaim is still required after current-session cleanup
- Need about 3.5G reclaimed to reach <=80% on /
  All writable stale /tmp paths total 15M across 136 path(s); short by 3.4G
  Review top stale /tmp paths: python3 tools/infra_tmp_cleanup.py --limit 5
  Current scan found 136 writable stale /tmp path(s); rerun the helper with a higher --limit or targeted --path values before cleanup
  Host-level reclaim is still required after current-session cleanup
...
- 0B sh pid=443994 19405 /tmp/#19405 (deleted)
- 0B sh pid=443994 19405 /tmp/#19405 (deleted)
