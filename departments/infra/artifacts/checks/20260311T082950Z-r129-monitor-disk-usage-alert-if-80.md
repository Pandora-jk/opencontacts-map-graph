...
- UTC: 2026-03-11T08:29:50.766601Z

## Disk Usage
Root usage: /: 97% used (18G/18G, avail 717M)
...
- Need about 1.1G reclaimed to reach <=90% on /
  All writable stale /tmp paths total 35M across 3 path(s); short by 1.1G
  Review top stale /tmp paths: python3 tools/infra_tmp_cleanup.py --limit 3
  Host-level reclaim is still required after current-session cleanup
- Need about 3.0G reclaimed to reach <=80% on /
  All writable stale /tmp paths total 35M across 3 path(s); short by 2.9G
  Review top stale /tmp paths: python3 tools/infra_tmp_cleanup.py --limit 3
  Host-level reclaim is still required after current-session cleanup
...
- Need about 3.0G reclaimed to reach <=80% on /
  All workspace logs total 5.3M across 2 path(s); short by 2.9G
  Review remaining workspace logs: python3 tools/infra_workspace_log_cleanup.py --path /home/ubuntu/.openclaw/workspace/logs/night-infra.log --path /home/ubuntu/.openclaw/workspace/logs/night-coding.log
  Host-level reclaim is still required after workspace-log cleanup
...
- Need about 3.0G reclaimed to reach <=80% on /
  All workspace caches total 236K across 1 path(s); short by 2.9G
  Review remaining workspace caches: python3 tools/infra_workspace_cache_cleanup.py --path /home/ubuntu/.openclaw/workspace/tools/__pycache__
  Host-level reclaim is still required after workspace-cache cleanup
Allowlisted home-cache recovery plan:
- Need about 1.1G reclaimed to reach <=90% on /
  All allowlisted home caches total 300M across 2 path(s); short by 844M
...
- Need about 1.1G reclaimed to reach <=90% on /
  All host-level caches/logs total 776M across 2 path(s); short by 367M
